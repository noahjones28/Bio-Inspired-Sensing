...
for i=i_jact
    ...
    elseif VLinks(LinkIndex(i)).jointtype=='H'
        ...
        if WrenchControlled(i_qj)
            ...
            B        = S.Vtwists(f).B;
            pitch    = sum(B(4:6));
            uq(i_qj) = eval(answer{1})+pitch*eval(answer{2});
        else
    ...
    elseif VLinks(LinkIndex(i)).jointtype=='C'
        ...
        if WrenchControlled(i_qj)
            ...
            opts.Interpreter = 'tex';
            answer           = inputdlg(prompt,dlgtitle,[1 100],definput,opts);
            uq(i_qj)    = eval(answer{1});
            uq(i_qj+1)  = eval(answer{2});
        else
        ...
        i_qj=i_qj+2;
    elseif VLinks(LinkIndex(i)).jointtype=='A' %planar
    ...
    elseif VLinks(LinkIndex(i)).jointtype=='F'
        ...
        if WrenchControlled(i_qj)
            ...
            uq(i_qj+1)  = eval(answer{2});
            uq(i_qj+2)  = eval(answer{3});
            uq(i_qj+3)  = eval(answer{4});
            uq(i_qj+4)  = eval(answer{5});
            uq(i_qj+5)  = eval(answer{6});
        else
