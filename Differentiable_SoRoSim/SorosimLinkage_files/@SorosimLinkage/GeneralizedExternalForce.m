...
%Last modified by Noah - 06/06/2025
...
for i=1:N
    ...
    J_here         = Ad_g_joint_inv*(J_here+TgPhi_here);
    ...
    for j=1:Tr.VLinks(Tr.LinkIndex(i)).npie-1 %will run only if soft link
        ...
        for ii=2:nip
            ...
            if Tr.Z_order==4
                ...
                Phi_Z1here  = Tr.CVRods{i}(j+1).Phi_Z1(6*(ii-2)+1:6*(ii-1),:);%note this step
                Phi_Z2here  = Tr.CVRods{i}(j+1).Phi_Z2(6*(ii-2)+1:6*(ii-1),:);
                ...
                if dof_here>0
                    ...
                    BGamma_here  = (H/2)*(Phi_Z1here+Phi_Z2here)+...
                ...
                B_Zhere  = Tr.CVRods{i}(j+1).B_Z(6*(ii-2)+1:6*(ii-1),:);%note this step
