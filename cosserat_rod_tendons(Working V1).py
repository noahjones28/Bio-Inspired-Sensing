...
class CoupledCosseratRod:
    ...
    def _ode_system(self, s, y, tau1, tau2, F_ext, s_force):
        ...
        if abs(s - s_force) < self.segment_length / 2 and F_ext != 0:
            ...
            # Perpendicular direction: [-sin(theta), cos(theta)] (90° CCW rotation)
        ...
        cross_product_z = px_dot * ny - py_dot * nx
        ...
        def shooting_residual(initial_guess):
            ...
            if force_applied:
                ...
                try:
                    ...
                    # Force direction: perpendicular to rod (downward relative to rod)
                    force_direction = np.array([-sin_theta, cos_theta])
        ...
        if show_plot:
            ...
            if sol_final is not None and sol_final.success:
                ...
                if s_idx < len(sol_final.y[0]) and F != 0:
                    ...
                    ax.plot(force_x, force_y, 'ro', markersize=8, 
                           label=f'Force application (s={s:.3f}m)')
                    
                    # Draw force arrow perpendicular to rod segment
                    arrow_length = 0.02  # 2cm arrow
                    force_direction = np.array([-np.sin(force_theta), np.cos(force_theta)])
                    arrow_end = arrow_length * force_direction
                    
                    ax.arrow(force_x, force_y, arrow_end[0], arrow_end[1],
...
if __name__ == "__main__":
    ...
    r_array = np.ones(20) * 0.0015  # 1.5mm radius, 20 segments
