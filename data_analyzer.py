...
class MyDataAnalyzer:
    def __init__(self):
        pass
    def compute_proximal_data(self, F_limits=np.array([0.1, 0.4]), s_limits=np.array([0.075, 0.2]), sigma_limits=np.array([0, 0.05]), tau_limits=np.array([0, 1.5]), matlab=eng):
        F_array = np.random.uniform(F_limits[0], F_limits[1], size=50)
        s_array = np.random.uniform(s_limits[0], s_limits[1], size=50)
        sigma_array = np.random.uniform(sigma_limits[0], sigma_limits[1], size=50)
        Tau_array = np.random.uniform(tau_limits[0], tau_limits[1], size=50)
        distal_values = np.stack((F_array, s_array, sigma_array, Tau_array), axis=1)
    ...
    def validate_inverse_model(self, proximal_values, target_distal_values, mean_error=np.zeros(3), std_error=np.zeros(3), n_trials=1, plot_results=True, matlab=eng):
        ...
        delta_sigma_array = np.zeros((len(target_distal_values), n_trials))
        ...
        delta_sigma_mean = np.mean(delta_sigma_array, axis=1)
        delta_sigma_stdev = np.std(delta_sigma_array, axis=1, ddof=1)

        # Flatten arrays to combine all points and trials
        all_delta_F = delta_F_array.flatten()
        all_delta_s = delta_s_array.flatten()
        all_delta_sigma = delta_s_array.flatten()
        ...
        delta_sigma_mean_overall = np.mean(all_delta_sigma)
        delta_sigma_stdev_overall = np.std(all_delta_sigma, ddof=1)
        ...
        print(f"Mean Δsigma: {delta_sigma_mean_overall:.6f}")
        print(f"Standard Deviation Δsigma: {delta_sigma_stdev_overall:.6f}")
        # Create heatmaps
        if plot_results:
            ...
            self.plot_heatmap(x_array=s_array, y_array=F_array, var1_mean=delta_F_mean, var1_stdev=delta_F_stdev,
                              var2_mean=delta_s_mean, var2_stdev=delta_s_stdev, var3_mean=delta_sigma_mean,
                              var3_stdev=delta_sigma_stdev, var1_title="ΔF", var2_title="Δs", var3_title="Δsigma",
                              x_label="s (m)", y_label="F (N)")

        return delta_F_mean_overall, delta_s_mean_overall, delta_sigma_mean_overall, delta_F_stdev_overall, delta_s_stdev_overall, delta_sigma_stdev_overall
...
proximal = np.array(proximal[:20])
distal = np.load('distal_values.npy')
distal = np.array(distal[:20])
