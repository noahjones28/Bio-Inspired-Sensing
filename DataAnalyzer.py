...
class DataAnalyzer:
    def __init__(self):
        pass
    def collect_samples(self, F_limits=np.array([0.1, 0.4]), s_limits=np.array([0.075, 0.2]), sigma_limits=np.array([0, 0.05]), tau_limits=np.array([0, 1.5]), n_samples=10):
        ...
        sigma_array = np.random.uniform(sigma_limits[0], sigma_limits[1], size=n_samples)
    ...
    def validate_inverse_model(self, proximal_values, target_distal_values, mean_error=np.zeros(6), std_error=np.zeros(6), n_trials=3, plot_results=True):
        ...
        all_delta_sigma = delta_sigma_array.flatten()
